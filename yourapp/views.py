...
def debug_mail(request):
    ...
    log = []
    log.append(f"DEBUG: Checking Mailjet Configuration...")
    
    api_key = getattr(settings, 'MAILJET_API_KEY', None)
    api_secret = getattr(settings, 'MAILJET_API_SECRET', None)
    
    if api_key:
        log.append(f"API Key found: {str(api_key)[:4]}...{str(api_key)[-4:]}")
    else:
        log.append("ERROR: API Key NOT found in settings.")
        
    if api_secret:
        log.append("API Secret found.")
    else:
        log.append("ERROR: API Secret NOT found in settings.")
              ...
              'Messages': [
                {
                  ...
                  "To": [
                    {
                      "Email": "danieljunior.business@gmail.com",
                      "Name": "Admin"
                    }
                  ],
                  "Subject": "Debug Email from Live Site",
                  "TextPart": "If you receive this, the live site can send emails.",
                }
              ]
            }
            log.append("Attempting to send email...")
            result = mailjet.send.create(data=data)
            log.append(f"Mailjet Status: {result.status_code}")
            log.append(f"Mailjet Response: {result.json()}")
        except Exception as e:
